...
Obviously, the image AI models can't be reproduced without the training data. If, however, you want to check 
out my inference scripts, install the Ultralytics package[^3] and go ahead! (A probably hopelessly inflated pip freeze
of my conda environment for these scripts can be found at [dsr-setup-requirements.txt](dsr-setup-requirements.txt).)

The [video counting script](src/segment_tracking_counting.py) can be used with the [raw example videos](RawVideos). 
Just adjust the file names and the counting line, if necessary. The [webcam script](src/webcam_counting.py) can be
used with off-the-shelf gherkins, fresh or pickled. (If the numbers seem off, try using the latest version 
of the Ultralytics software.)
